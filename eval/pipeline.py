...
def run(examples: list[EvalExample], index, records: list[ChunkRecord], top_k: int, workers: int) -> list[ExampleResult]:
    ...
    embedding_backend = target.optional_config("EMBEDDING_BACKEND", default=None)
    ...
    elif embedding_backend == "local" and workers > 1:
        effective_workers = 1
        print(
            f"[pipeline] EMBEDDING_BACKEND=\"local\" -- clamping workers {workers} -> 1 "
            f"(shared embedding model load is not thread-safe on this target)."
        )
