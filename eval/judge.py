...
def _call_openai(system_prompt: str, user_content: str) -> JudgeVerdict:
    ...
    try:
        ...
        raise JudgeNotConfigured(f"OpenAI judge auth failed: {e}") from e
    except openai.RateLimitError as e:
        raise JudgeNotConfigured(
            "OpenAI judge unavailable (quota or billing). "
            "Retrieval, reliability, and latency checks still run without it."
        ) from e
